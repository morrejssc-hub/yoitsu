...
| `git+file://` / `git+ssh://` / `git+https://` / `https://github.com/org/repo.git` | Git 托管 | Bundle 仓库快照、代码产物 |
...
| **artifact URI 指向** | 不产生 artifact | `git+https://repo@sha` 指向远端 |
...
- Workspace **永远不是权威**
- Artifact URI 必须指向远端仓库（如 `git+https://github.com/org/repo@sha`），不能指向 ephemeral workspace 路径
