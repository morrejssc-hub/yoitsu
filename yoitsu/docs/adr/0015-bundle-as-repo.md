...
**Hallucination gate 是 `git diff` 的事实检测**：staging 非空即 commit+push；staging 为空即 publication 跳过（`publication.skipped`）。不做 whitespace 过滤、不做语义判断。"变更是否解决了 goal" 以及 "没有变更是否算失败" 均由 evaluator role 独立回答。
...
1. **变更存在性 gate（hallucination gate）**：`git diff --cached --quiet` 检测 agent 是否真的改了文件。无变更时 publication 跳过（`publication.skipped`），但 **不决定 job 成败**——"没改文件算不算失败"是第 3 层（evaluator）的判断。不做 whitespace 过滤、不做语义判断。
...
def finalize(self, ctx: JobContext) -> FinalizeResult:
    ...
    if result.returncode == 0:  # 无变更
        ...
        return FinalizeResult(events=events, success=True)  # 无变更不是失败
    
    # 容器环境需要 git identity
    subprocess.run(["git", "config", "user.email", "yoitsu@example.com"], cwd=ctx.target_workspace)
    subprocess.run(["git", "config", "user.name", "Yoitsu Bot"], cwd=ctx.target_workspace)
    
    # Commit + 重试 push
    subprocess.run(["git", "commit", "-m", f"job: {ctx.job_id}"], cwd=ctx.target_workspace)
    sha = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ctx.target_workspace).decode().strip()
    
    MAX_RETRIES = 3
    for attempt in range(MAX_RETRIES):
        try:
            subprocess.run(["git", "push"], cwd=ctx.target_workspace, check=True)
            # artifact URI 指向远端仓库，使用 git+ scheme
            events.append(EventData(type="artifact.published", data={
                "ref": f"git+{ctx.target_source.repo_uri}@{sha}",
...
**Git Workspace Finalize 语义**：

| 检出结果 | capability 返回 | 说明 |
|---|---|---|
| 有变更，push 成功 | `success=True` | artifact.published 事件 |
| 有变更，push 失败 | `success=False` | finalize.failed 事件 |
| 无变更 | `success=True` | publication.skipped 事件 |

**关键**：git_workspace capability 只管 git 生命周期——有变更就 commit+push，无变更就跳过。"没改文件算不算达成 goal"是 evaluator 的判断（§2.5 第 3 层），不是 capability 的职责。
